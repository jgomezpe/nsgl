...
public abstract class Function<S, T> implements Runnable{
	...
	public Iterable<T> set_apply( Iterable<?> set ){
    	...
    	if( Tagged.class.isAssignableFrom(cl) )
    		return new Cast<Tagged<S>,T>( (Iterable<Tagged<S>>)set ){	@Override public T cast( Tagged<S> x ) { return apply(x); } };
		else
			return new Cast<S,T>( (Iterable<S>)set ){	@Override public T cast( S x ) { return apply(x); } };
	}
