...
public interface Manager<T> {
	...
	default Iterable<T> unwrap( Iterable<Tagged<T>> col ){
		return new Cast<Tagged<T>,T>(col){ @Override protected T cast(Tagged<T> x) { return unwrap(x); } };
	}
	
	default Iterable<Tagged<T>> wrap( Iterable<T> col ){
		return new Cast<T,Tagged<T>>(col){ @Override protected Tagged<T> cast(T x) { return wrap(x); } };
	}
}